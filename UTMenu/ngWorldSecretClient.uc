...
function Created()
{
	...
	SecretCombo.AddItem(GetPlayerOwner().GetNGSecret());
...
function Close(optional bool bByParent)
{
	...
	GetPlayerOwner().SetNGSecret(SecretCombo.GetValue());
...
function OKPressed()
{
	if (SecretCombo.GetValue() == GetPlayerOwner().GetNGSecret())
		Close();
...
function MessageBoxDone(UWindowMessageBox W, MessageBoxResult Result)
{
	if(W == ConfirmChange)
	{
		...
		case MR_Yes:
			...
			break;
		case MR_No:
			SecretCombo.SetValue(GetPlayerOwner().GetNGSecret());
			break;
		}				
	}
}
